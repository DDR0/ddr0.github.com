...
(function() {
  var addNewRoadTiles, addTile, advanceRoadTileAgent, c, cacheEaselValues, cityGen, closestTile, configureCanvas, groundLayer, handleComplete, newTile, queue, roadTileAgents, setCallbacks, stage, tileHeight, tileSheet, tileSheetRects, tileWidth, windowMidpoint,
  ...
  tileSheet = tileSheetRects = groundLayer = stage = windowMidpoint = handleComplete = function() {
  ...
  addNewRoadTiles = function(walked) {
    ...
    for (_i = 0, _len = walked.length; _i < _len; _i++) {
      ...
      _results.push(addTile(tile.x, tile.y, 'road' + (tile.connects[1] * 1 || '') + (tile.connects[3] * 3 || '') + (tile.connects[7] * 7 || '') + (tile.connects[9] * 9 || '')));
    }
    return _results;
  };

  cityGen = function(startTile) {
    var advanceAgent, agent, agentNo, agents, branchChance, branchType, crossedExistingPath, newAgents, oldAgents, reverse, step, turnChance, turnTo, turnType, walk, walked, _i, _j, _len, _ref;
    ...
    for (step = _i = 1, _ref = Math.min(startTile.x, startTile.y) / 2; 1 <= _ref ? _i <= _ref : _i >= _ref; step = 1 <= _ref ? ++_i : --_i) {
